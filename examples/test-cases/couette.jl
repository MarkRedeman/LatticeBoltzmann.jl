...
function couette_velocity_profile(
    ...
    simulate(
        ...
        process_method = ShowVelocityError(
            problem, plot_every, Float64[], LatticeBoltzmann.VelocityConvergenceStoppingCriteria(1E-7, problem)
        ),
    )
end
...
function plot_snapshots(problem, snapshot_results, snapshots, q)
    ...
    for (time, snapshot) in zip(snapshots, snapshot_results)
        ...
        scatter!(velocity_profile_x, domain, abs.(u[x_pos, :, 1]), label = latexstring("t = ", timestep), markershape = marker, markercolor = :gray, markersize=8)
