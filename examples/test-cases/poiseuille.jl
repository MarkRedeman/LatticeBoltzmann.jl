...
function poiseuille_convergence_analysis(
    q = D2Q9(),
...
function main2()
    ...
    results = map(quadratures) do q
        poiseuille_velocity_profile(q, ZeroVelocityInitialCondition(), τ, scale)
    end
    plot_error_locations(results) |> display

    results = map(quadratures) do q
        poiseuille_velocity_profile(q, ZeroVelocityInitialCondition(), τ, 2scale)
    end
    plot_error_locations(results) |> display

    results = map(quadratures) do q
        poiseuille_velocity_profile(q, ZeroVelocityInitialCondition(), τ, 4scale)
    end
    plot_error_locations(results) |> display
end
...
function main_convergence(τ = 0.8)
    ...
    convergence_results = map(quadratures) do q
        poiseuille_convergence_analysis(q, ZeroVelocityInitialCondition(), τ, scales = scales)
    end
...
function main(τ = 1.0, scale = 2)
    ...
    Δt = delta_t(problem)
    snapshot_at =
        round.(Int, [0.005 * 1.0 / (ν * Δt), 0.05 * 1.0 / (ν * Δt), 5.0 * 1.0 / (ν * Δt)])
    ...
    convergence_results = map(quadratures) do q
        poiseuille_convergence_analysis(q, iteration_strategies[1], τ, scales = scales)
    end
    convergence_results_iterative = map(quadratures) do q
        poiseuille_convergence_analysis(q, iteration_strategies[2], τ, scales = scales)
    end
    convergence_results_equilibrium = map(quadratures) do q
        poiseuille_convergence_analysis(q, iteration_strategies[3], τ, scales = scales)
    end
    convergence_results_offequilibrium = map(quadratures) do q
        poiseuille_convergence_analysis(q, iteration_strategies[4], τ, scales = scales)
    end
