    ...
    map(
        scale -> begin
            result = LatticeBoltzmann.simulate(
                DecayingShearFlow(
                    1.000 / (q.speed_of_sound_squared * 2),
                    scale,
                    static = false,
                ),
                q,
...
result = LatticeBoltzmann.simulate(
    TaylorGreenVortex(τ, scale, static = false),
    q,
    t_end = 0.1250,
    should_process = true,
);
...
for scale in scales
    ...
    result = LatticeBoltzmann.simulate(problem, q, t_end = 1.0, should_process = false)
end
...
@time x =
    map(
        scale -> begin
            ...
            problem =
                    TaylorGreenVortex(ρ_0, u_max, ν, NX, NY, (2pi, 2pi), false, A, B, a, b)
