...
import LatticeBoltzmann:
    density, velocity!, temperature, pressure, momentum_flux, deviatoric_tensor
...
for q in LatticeBoltzmann.Quadratures
    ...
    suite[string(q)]["density"] =
        @benchmarkable(density($q, f), setup = (f = copy($q.weights)))

    suite[string(q)]["velocity"] = @benchmarkable(
        velocity!($q, f, ρ, u),
    ...
    suite[string(q)]["pressure"] = @benchmarkable(
        pressure($q, f, ρ, u),
        setup = (f = copy($q.weights);
        ρ = density($q, f);
        u = zeros(eltype(f), dimension($q));
    ...
    suite[string(q)]["temperature"] = @benchmarkable(
        pressure($q, f, ρ, u),
        setup = (f = copy($q.weights);
        ρ = density($q, f);
        u = zeros(eltype(f), dimension($q));
    ...
    suite[string(q)]["momentum_flux"] = @benchmarkable(
    ...
    suite[string(q)]["deviatoric_tensor"] = @benchmarkable(
    ...
    suite[string(q)]["density", "static"] = @benchmarkable(
        density(static_q, f),
        setup = (
    ...
    suite[string(q)]["velocity", "static"] = @benchmarkable(
